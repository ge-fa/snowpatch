...
pub static PATCHWORK_QUERY: &'static str = "?order=-id&per_page=125";
...
impl PatchworkServer {
    ...
    pub fn get_project(&self, url: &str) -> Result<Project, DecoderError> {
    ...
    pub fn get_patch_by_url(&self, url: &str) -> Result<Patch, DecoderError> {
        json::decode(&self.get_url_string(url).unwrap())
    }
    ...
    pub fn get_series_by_url(&self, url: &str) -> Result<Series, DecoderError> {
        json::decode(&self.get_url_string(url).unwrap())
    }
